...
class MFIDA(nn.Module):
    def __init__(self):
        ...
        self.Focus = common.PFocus()
        self.CBS1 = common.Tconvolutional(filters_shape=(3, 3, 12, 32), downsample=True, activate_type="leaky")
        self.CBS2 = common.Tconvolutional(filters_shape=(3, 3, 32, 16), downsample=True, activate_type="leaky")
        self.Involution2d = Involution2d.Involution2d(in_channels=16, out_channels=8, groups=2, activate_type="leaky")
        self.Attention = mAttention.MultiSpectralAttentionLayer(8, 8, 8, activate_type="leaky")
...
print("device:",device)
print()
mFida = MFIDA().to(device)
summary(mFida, input_size=(3, 64, 64))


# %%
CUDA = True
device_test = torch.device("cuda:0" if CUDA and torch.cuda.is_available() else "cpu")
model = MFIDA().to(device_test)
...
model_path = r"save_model\A1-38.pth"
mFida = torch.load(model_path)
acc_list, total_acc_list, erro_list = [], [], []
