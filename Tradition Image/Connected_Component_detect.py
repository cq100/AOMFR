...
for i in range(N):
    ...
    _ = EN.predict(path, show=False, debug=False)
    end_time = time.time()
    T.append(round(1000*(end_time-start_time),3))
print("连通域耗时：{} ms".format(round(sum(T[10:])/len(T[10:]),3)))


# %%
path = r"..\Dataset\IMG\No.str(1)\6.jpg"
EN.predict(path,debug=True)

# %%
# EN.centers
_ = EN.draw_point(EN.centers,EN.binary,save=True)
