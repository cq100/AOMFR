...
class Image_process(object):
    ...
    def Fast_predict(self,path,show=True,debug=False):
        ...
        if show:
            ...
            EN.Image_show(img2)
            cv2.imwrite('Fast.jpg',img2, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
...
class Evaluation_Norm(Image_process):
    ...
    def evaluation(self,paths,type="fast"):
        pred = []
        ...
        for index in tqdm.tqdm(range(1,101),desc="Evaluation"): 
            ...
            if os.path.exists(path):
                if type == "canny":
                    ...
                    raise ValueError("type must be 'canny' or 'Connected' or 'fast' !") 
                res = res.reshape(16,)
                pred.append(res)
        return pred
...
# %%
path = r"IMG\2.jpg"
EN.Fast_predict(path,show=True)
...
#     pred = EN.evaluation(path_img,type="fast")
...
# %%
EN.draw_result(acc_list,total_acc_list,save=False)
...
# %%
path = r"..\Dataset\IMG\No.str(1)\8.jpg"

N = 100
T = []
for i in range(N):
    start_time = time.time()
    _ = EN.Fast_predict(path,show=False)
    end_time = time.time()
    T.append(round(1000*(end_time-start_time),3))
print("FAST 耗时：{} ms".format(round(sum(T[10:])/len(T[10:]),3)))

# %%
path = r"..\Dataset\IMG\No.str(1)\1.jpg"
EN.Fast_predict(path,show=True)
# EN.predict(path,debug=False)

# %%
img2 = EN.read_image(path)
img3 = EN.Image_binary(img2,threshold=150)
EN.Image_show(img3,gray=True)

# %%
# EN.centers
_ = EN.draw_point(EN.centers,EN.binary)

# %%
cc = []
kk = []
for i in range(len(EN.ori_center)):
    if i not in [0,8]:
        cc.append([int(EN.ori_center[i][1]),int(EN.ori_center[i][0])])
    kk.append([int(EN.ori_center[i][1]),int(EN.ori_center[i][0])])

_ = EN.draw_point(cc,EN.binary)
