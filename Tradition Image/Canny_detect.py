...
class Image_process(object):
    ...
    def Canny_predict(self,path,show=True,debug=False):
        ...
        if show:
            ...
            cv2.imwrite('edge.jpg', edge, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
...
class Evaluation_Norm(Image_process):
    ...
    def evaluation(self,paths,type="canny"):
        pred = []
        ...
        for index in tqdm.tqdm(range(1,101),desc="Evaluation"): 
            ...
            if os.path.exists(path):
                if type == "canny":
                    ...
                    raise ValueError("type must be 'canny' or 'Connected' !") 
                res = res.reshape(16,)
                pred.append(res)
        return pred
...
#     pred = EN.evaluation(path_img,type="canny")
...
# %%
path = r"..\Dataset\IMG\No.str(1)\7.jpg"

N = 100
T = []
for i in range(N):
    start_time = time.time()
    _ = EN.Canny_predict(path,show=False)
    end_time = time.time()
    T.append(round(1000*(end_time-start_time),3))
print("Canny耗时：{} ms".format(round(sum(T[10:])/len(T[10:]),3)))
